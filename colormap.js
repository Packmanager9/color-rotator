...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    function setUp(canvas_pass, style = "gray") {
        canvas = canvas_pass
    ...
    let addmore = document.getElementById('more') //getting canvas from document
    let clearer = document.getElementById('less') //getting canvas from document
    ...
    tpgif.src = "dd.png"
    addmore.onclick = addmorelol
    clearer.onclick = clearoff
    class Framething {
        constructor() {
            this.frame = 0
            this.framemod = 1
        ...
        draw() {
            // this.frame++
            // if (this.frame % this.framemod == 0) {
            //     this.frame = 0
            // }

            // let srx = 0 + ((tpgif.width / this.framemod) * this.frame)
            let srx = 0

            canvas_context.drawImage(tpgif, srx, 0, (tpgif.width / this.framemod), tpgif.height, 0, 0, canvas.width, canvas.height)
    ...
    window.addEventListener('load', function() {
        document.querySelector('input[type="file"]').addEventListener('change', function() {
                tpgif.src = URL.createObjectURL(this.files[0]); // set src to blob url
      ...
      window.addEventListener('load', function() {
          document.querySelector('input[type="text"]').addEventListener('change', function() {
                 canvas.style.background = colorer.value
    ...
    function main() {
        if(keysPressed['k']){
            ...
            canvas_context.drawImage(tpgif, 0,0)
    
            rspin+=(.005*Math.random())*255
            gspin+=(.010*Math.random())*255
            bspin+=(.020*Math.random())*255
            gamepadAPI.update() //checks for button presses/stick movement on the connected controller)
            // game code goes here
            const imageData = canvas_context.getImageData(0, 0, canvas.width, canvas.height);
            const data = imageData.data;
            for (var i = 0; i < data.length; i += 4) {
                data[i]  = (data[i]+rspin)%255
                data[i + 1] =  (data[i+1]+gspin)%255
                data[i + 2] =  (data[i+2]+bspin)%255
            }
            ...
            link.download = `scream${globalx}.png`
